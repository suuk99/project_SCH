...
<script>
	...
	const loginIdChk = function(el) {
		...
		if (el.value.length == 0) {
			msg.removeClass('text-green-500').addClass('text-red-500').html('아이디를 입력하세요.');
			return;
		}
		...
		$.ajax({
			...
			type: 'get',
			data: { userId: el.value },
			dataType: 'json',
			success: function(data) {
				if (data.rsCode.startsWith("S-")) {
					msg.removeClass('text-red-500').addClass('text-green-500').html(data.rsMsg);
					validLoginId = el.value;
				} else {
					msg.removeClass('text-green-500').addClass('text-red-500').html(data.rsMsg);
					validLoginId = null;
				}
			}
		});
	...
	const passwordChk = function(el) {
		...
		if (el.value.length == 0) {
			msg.removeClass('text-green-500').addClass('text-red-500').html('비밀번호를 입력하세요.');
			return;
		}
	...
	const bdChk = function(el) {
		...
		if (el.value.length == 0) {
			msg.removeClass('text-green-500').addClass('text-red-500').html('생년월일을 입력하세요.');
			return;
		}
	...
	const pnChk = function(el) {
		...
		if (el.value.length == 0) {
			msg.removeClass('text-green-500').addClass('text-red-500').html('전화번호를 입력하세요.');
			return;
		}
		...
		$.ajax({
			...
			success: function(data) {
				if(data.rsCode.startsWith("S-")) {
					msg.removeClass('text-red-500').addClass('text-green-500').html('');
				} else {
					msg.removeClass('text-green-500').addClass('text-red-500').html(data.rsMsg);
				}
			}
